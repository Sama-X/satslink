...
dfx deploy --network=$network satslinker --argument "()"

if [ $mode = dev ]; then
    dfx deploy --network=$network internet_identity 

    dfx deploy --network=$network satslink_token --argument "(variant { Init = record { \
        token_symbol = \"SATSLINK\"; \
        token_name = \"MSQ Cycle Satslink\"; \
        minting_account = record { owner = principal \"$CAN_SATSLINKER_CANISTER_ID\"  }; \
        transfer_fee = 10_000; \
    ...
    dfx canister update-settings --network=$network satslink_token --add-controller r7inp-6aaaa-aaaaa-aaabq-cai
fi
